...
struct tagScoreInfo
{
	...
	SCORE							lGrade;								//用户成绩
	SCORE							lRevenue;							//游戏税收
	std::vector<BYTE>               kHandCardCost;                      //用户牌型
};
...
public:
	...
	virtual SCORE getPlayScore(int index)=NULL;//返回用户总分数

	virtual WORD getChairID(int index)=NULL;//返回用户ID

	virtual SCORE getHighScore(int index)=NULL;//返回用户单局最高分

	virtual WORD getWinNum(int index)=NULL;//返回用户赢局数量

	virtual WORD getBoombNum(int index)=NULL;//返回用户打出炸弹数量
...
public:
	//复位接口
	virtual VOID RepositionSink(bool resizeScore = false)=NULL;
	//配置接口
