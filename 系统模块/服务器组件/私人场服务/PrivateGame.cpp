...
int PriaveteGame::getRoomCoutByCreaterID(DWORD dwUserID)
{
	...
	for (int i = 0;i<m_pGameServiceOption->wTableCount;i++)
	{
		if (m_pTableInfo[i].pITableFrame && m_pTableInfo[i].dwCreaterUserID == dwUserID)
		{
			iRoomCout ++;
		}
	}
...
bool PriaveteGame::OnEventCreatePrivate(WORD wRequestID, IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize,std::string kChannel)
{
	...
	if (getRoomCoutByCreaterID(pIServerUserItem->GetUserID()) > 10)
	{
		m_pIGameServiceFrame->SendRoomMessage(pIServerUserItem,TEXT("你已经创建超过10个房间，不能再创建。"),SMT_EJECT);
		return true;
	}
	...
	else
	{
		...
		if (pTableInfo)
		{
			...
			m_pIGameServiceFrame->SendRoomMessage(pIServerUserItem,TEXT("你已经在游戏中，不能再创建。"),SMT_EJECT);
			return true;
		}
		WORD wTableID = 0;
		for (wTableID=0;wTableID<m_pGameServiceOption->wTableCount;wTableID++)
		{
			...
			PrivateTableInfo* pPrivateInfo = &m_pTableInfo[wTableID];
	...
	{
		m_pIGameServiceFrame->SendRoomMessage(pIServerUserItem,TEXT("创建房间失败。"),SMT_EJECT);
		return true;
	}
	...
	int iRandNum = 0;
	while (iRandNum == 0 || getTableInfoByRoomID(iRandNum))
	{
...
bool PriaveteGame::OnTCPNetworkSubCreatePrivate(VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	...
	//创建私人房
	if(pCMDInfo->cbGameType != Type_Public)
	{
		...
		DWORD d = (DWORD)m_kPrivateInfo.lPlayCost[pCMDInfo->bPlayCoutIdex];
...
bool PriaveteGame::OnTCPNetworkSubDismissPrivate(VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	...
	//必须两个人以上同意才能解散
	if (!pTableInfo->bStart || (int)kNetInfo.dwDissUserCout >= 2)
	{
		bClearDismissInfo = true;
...
void PriaveteGame::DismissRoom(PrivateTableInfo* pTableInfo)
{
	...
	if (pTableInfo->bStart)
	{
		...
		kNetInfo.lChairIDs.resize(pTableFrame->GetChairCount());
		kNetInfo.lBombCount.resize(pTableFrame->GetChairCount());
		kNetInfo.lGameHighScore.resize(pTableFrame->GetChairCount());
		kNetInfo.lWinNum.resize(pTableFrame->GetChairCount());
		...
		for (int i = 0;i<pTableFrame->GetChairCount();i++)
		{
			...
			kNetInfo.lChairIDs[i] = pTableFrame->getChairID(i);
			kNetInfo.lPlayerWinLose[i] = pTableFrame->getPlayScore(i);
			kNetInfo.lBombCount[i] = pTableFrame->getBoombNum(i);
			kNetInfo.lGameHighScore[i] = pTableFrame->getHighScore(i);
			kNetInfo.lWinNum[i] = pTableFrame->getWinNum(i);
	...
	if (pTableFrame->IsGameStarted()==true)
	{
		...
		{
			//提示消息
		    CTraceService::TraceString("强制解散失败",TraceLevel_Exception);
			return;
		}
		else
		{
			isForceEndGame = true;
			//提示消息
		    CTraceService::TraceString("强制解散成功",TraceLevel_Exception);
		}
	}
	else
	{
