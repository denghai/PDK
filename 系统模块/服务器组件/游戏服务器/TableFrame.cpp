...
#define  GAME_RULE_TWO_GAME               5                             //两人玩法
#define  GAME_RULE_THREE_GAME             6                             //三人玩法
...
//返回用户总分数
SCORE CTableFrame::getPlayScore(int index)
{
	return m_pITableFrameSink->getPlayScore(index);
}

//返回用户单局最高分
SCORE CTableFrame::getHighScore(int index)
{
	return m_pITableFrameSink->getHighScore(index);
}

//返回用户的ChairID
WORD CTableFrame::getChairID(int index)
{
	return m_pITableFrameSink->getChairID(index);
}

//返回用户赢局数量
WORD CTableFrame::getWinNum(int index)
{
	return m_pITableFrameSink->getWinNum(index);
}

//返回用户打出炸弹数量
WORD CTableFrame::getBoombNum(int index)
{
	return m_pITableFrameSink->getBoombNum(index);
}
...
bool CTableFrame::DismissGame()
{
	...
	//清理分数
	//提示消息
	m_pITableFrameSink->RepositionSink(true);
...
{
	//判断是否为两人玩法
	bool isTwo = hasRule(bGameRuleIdex,GAME_RULE_TWO_GAME);
	if(isTwo)
	{
		m_wChairCount = 2;
	}
...
bool CTableFrame::hasRule(DWORD m_dwGameRule,BYTE cbRule)
{
