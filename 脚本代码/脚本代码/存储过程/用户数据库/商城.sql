...
BEGIN
	...
	-- 查询用户
	DECLARE @DestUserID INT
	DECLARE @DestNickName AS NCHAR(32)
	SELECT @DestUserID=UserID,@DestNickName=NickName
	FROM AccountsInfo(NOLOCK) WHERE UserID=@dwDestUserID
	-- 查询用户
	IF @DestUserID IS NULL
	BEGIN
...
-- 查询资料
CREATE PROC GSP_GP_GetSellRecordList
	@dwUserID INT							-- 用户帐号
...
-- 执行逻辑
BEGIN
	DECLARE @DestUserID AS INT
	DECLARE @ItemValue AS INT
	DECLARE @OderDate AS datetime
	DECLARE @DestUserName NVARCHAR(32) 
	
	DECLARE @tempRankBack TABLE
	(
      DestUserID INT,
      DestUserName NVARCHAR(32) ,
      OderDate datetime,
      ItemValue INT
	);
		-- 声明游标
	DECLARE C_Employees CURSOR FAST_FORWARD FOR
		SELECT  TOP 20 DestUserID,DestUserName,OderDate,ItemValue
		FROM QPRecordDBLink.QPRecordDB.dbo.UserSellOder
		WHERE SrcUserID = @dwUserID
		ORDER BY OderDate DESC
		
	OPEN C_Employees;

	-- 取第一条记录
	FETCH NEXT FROM C_Employees INTO @DestUserID,@DestUserName,@OderDate,@ItemValue;

	WHILE @@FETCH_STATUS=0
	BEGIN
		
		SELECT @DestUserName=NickName FROM AccountsInfo(NOLOCK) WHERE UserID=@DestUserID
		
		IF @DestUserName IS NOT NULL
		BEGIN
			INSERT INTO @tempRankBack ([DestUserID],[DestUserName], [OderDate],[ItemValue]) VALUES (@DestUserID,@DestUserName,@OderDate,@ItemValue)
		END
